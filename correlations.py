    ...
    for label in INTERVALS_LABELS:
        ...
        for zone in Zones.get():
            if( zone < 41):
                ...
                z = str(zone)
                for q in range(1,6):
                    x_axis[q - 1].append(quintiles[z][q])
                    x_average[i] += float(quintiles[z][q]) * q
                x_average[i] = x_average[i] / 100
        for q in range(1,6):
            Printer.line_graph(x_axis[q - 1], y_axis, './correlations/quintile_' + str(q) + '/'+ gender + '_' + label + '.png')
        Printer.line_graph(x_average, y_axis, './correlations/quintile_average/'+ gender + '_' + label + '.png')
